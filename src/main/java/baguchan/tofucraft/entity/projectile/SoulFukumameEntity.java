...
public class SoulFukumameEntity extends ThrowableEntity {
	...
	public SoulFukumameEntity(EntityType<? extends SoulFukumameEntity> p_i50154_1_, World worldIn, double x, double y, double z) {
	...
	public void handleStatusUpdate(byte id) {
		if (id == 3) {
			double d0 = 0.08D;
			for (int i = 0; i < 6; ++i) {
				this.world.addParticle(new ItemParticleData(ParticleTypes.ITEM, new ItemStack(TofuItems.SEEDS_SOYBEANS_SOUL)), this.getPosX(), this.getPosY(), this.getPosZ(), ((double) this.rand.nextFloat() - 0.5D) * 0.08D, ((double) this.rand.nextFloat() - 0.5D) * 0.08D, ((double) this.rand.nextFloat() - 0.5D) * 0.08D);
			}
		} else if (id == 4) {
			double d0 = 0.08D;

			this.world.addParticle(ParticleTypes.WARPED_SPORE, this.getPosX(), this.getPosY(), this.getPosZ(), ((double) this.rand.nextFloat() - 0.5D) * 0.08D, ((double) this.rand.nextFloat() - 0.5D) * 0.08D, ((double) this.rand.nextFloat() - 0.5D) * 0.08D);
