...
public class LootEvents {
	...
	public static void onInjectLoot(LootTableLoadEvent event) {
		...
		if (SHIP_LOOT.contains(event.getName())) {
			LootPool pool = LootPool.lootPool().add(TableLootEntry.lootTableReference(new ResourceLocation(TofuCraftReload.MODID, "injections/tofustick_ship")).setWeight(1).setQuality(1)).name("tofustick_ship").build();
			event.getTable().addPool(pool);
		}

		if (RUIN_LOOT.contains(event.getName())) {
			LootPool pool = LootPool.lootPool().add(TableLootEntry.lootTableReference(new ResourceLocation(TofuCraftReload.MODID, "injections/tofustick_ruin")).setWeight(1).setQuality(1)).name("tofustick_ruin").build();
			event.getTable().addPool(pool);
		}
	}
}